...
export function StrichLernTrainer({ words, onRecord, onFinish }: Props) {
      ...
      {phase === 'animate' ? (
        <div className="flex flex-col items-center gap-2">
          <p className="text-sm text-gray-400 dark:text-gray-500 text-center">
            Schaue dir die Striche an, dann übe selbst
          </p>
          <button
            onClick={startQuiz}
            className="px-6 py-2 bg-purple-600 text-white rounded-full font-medium text-sm active:scale-95 transition-transform"
          >
            Jetzt üben ✍️
          </button>
        </div>
